...
cat("Mean Absolute Error:", scores$ae_point, "\n")
cat("Root Mean Squared Error:", sqrt(scores$se_point), "\n")
cat("Absolute Percentage Error:", scores$ape, "\n")
```
...
Use `score(forecast)` to compute all available metrics, then access them from the result:
...
coverage_50 <- scores$interval_coverage_50  # 50% interval coverage
coverage_90 <- scores$interval_coverage_90  # 90% interval coverage
```
