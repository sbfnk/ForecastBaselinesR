...
mae_score <- MAE(fc_with_truth)
rmse_score <- RMSE(fc_with_truth)
mape_score <- MAPE(fc_with_truth)

cat("Mean Absolute Error:", mae_score, "\n")
cat("Root Mean Squared Error:", rmse_score, "\n")
cat("Mean Absolute Percentage Error:", mape_score, "\n")

# Get all scores at once using scoringutils
...
**Note:** Scoring is now powered by the [scoringutils](https://epiforecasts.io/scoringutils/) package, providing standardized and comprehensive forecast evaluation.
...
- `WIS(forecast)` - Weighted Interval Score (requires quantiles/intervals)
- `CRPS(forecast)` - Continuous Ranked Probability Score (approximated via WIS for quantile forecasts)

**General Scoring:**

- `score(forecast)` - Compute all available metrics for the forecast type
- `score(forecast, summarise = FALSE)` - Get scores for each horizon separately
- `get_available_metrics("point")` - See all available point forecast metrics
- `get_available_metrics("quantile")` - See all available quantile forecast metrics
